...
export default function WayneDashboard({ onLogout }) {
    ...
    async function loadData() {
      ...
      try {
        ...
        setPlayerList(playerRows.map(r => {
          ...
          const programLast = pick(r, ["program_last", "Program (Last Yr)"]);
          const programThis = pick(r, ["program_this", "Program (This Yr)", "program"]);
          const program = programThis || programLast || "Unknown";
  ...
  const ageComparisonData = useMemo(() => {
    ...
    playerList.forEach(p => {
      ...
      // last = teamLast exists + not aged out (for retention base)
    ...
    .sort((a, b) => Number(b.year) - Number(a.year)) // Sort 2019 -> 2007
  ...
  const exactRevenueLost = useMemo(() => {
    // Only sum lost players who didn't age out
    ...
    // Sum their actual fees (use activeData.fee as fallback if 0)
  ...
  const deepDiveStats = useMemo(() => {
    ...
    // Calculate New for summary
    // Since we don't have explicit "New" count in 'teams' CSV, we estimate: Total - Retained.
    // If Total > Retained, diff is New. If Total < Retained (data error), New is 0.
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-700/30">
                    {filteredTeams.map((team, idx) => {
                      ...
                      // Calculate New based on current roster count - retained
                    ...
                    {deepDiveStats.teams.map((t, idx) => (
                      <div key={idx} className="p-5 bg-[#111827] rounded-2xl border border-slate-700/50">
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => handleOpenPlayerList({ team: t.name, status: 'Retained' }, `Retained: ${t.name}`)} className="flex-1 text-xs bg-blue-500/20 text-blue-400 px-3 py-2 rounded-lg font-bold">Retained: {t.retained}</button>
                          <button onClick={() => handleOpenPlayerList({ team: t.name, status: 'Lost' }, `Lost: ${t.name}`)} className="flex-1 text-xs bg-rose-500/20 text-rose-400 px-3 py-2 rounded-lg font-bold">Lost: {t.lost}</button>
                        </div>
                      </div>
                    ))}
                  </div>
